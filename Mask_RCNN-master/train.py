...
class ShapesConfig(Config):
    ...
    NUM_CLASSES = 1 + 4  # background + 1 shapes
    ...
    # 因为数据简单，使用小的epoch 每个ecophs循环次数

    STEPS_PER_EPOCH = 100

    # 因为epoch较小，使用小的交叉验证步数
    VALIDATION_STEPS = 30
...
class DrugDataset(utils.Dataset):
    ...
    def load_shapes(self, count, img_floder, mask_floder, imglist, dataset_root_path):
        ...
        self.add_class("shapes", 3, "maize")
        self.add_class("shapes", 4, "wheat")
    ...
    def load_mask(self, image_id):
        ...
        for i in range(len(labels)):
            if labels[i].find("soya") != -1:
                # print "car"
                labels_form.append("soya")
            elif labels[i].find("rice") != -1:
                # print "leg"
                labels_form.append("rice")
            elif labels[i].find("maize") != -1:
                # print "leg"
                labels_form.append("maize")
...
# 基础设置
dataset_root_path = "samples/my_data/"
...
elif init_with == "coco":#接着上一个训练的模型接着训练
